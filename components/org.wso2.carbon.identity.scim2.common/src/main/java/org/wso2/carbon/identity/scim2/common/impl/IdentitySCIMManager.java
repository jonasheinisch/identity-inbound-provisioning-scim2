...
public class IdentitySCIMManager {
	...
	/**
	 * Should return the static instance of CharonManager implementation. Read the
	 * config and initialize extensions as specified in the config.
	 *
	 * @return
	 */
	...
	private void init() throws CharonException {
		// this is necessary to instantiate here as we need to encode exceptions if they
		// occur.
		...
		endpointURLs.put(SCIMConstants.SERVICE_PROVIDER_CONFIG_ENDPOINT,
				SCIMCommonUtils.getSCIMServiceProviderConfigURL());
		endpointURLs.put(SCIMConstants.RESOURCE_TYPE_ENDPOINT, SCIMCommonUtils.getSCIMResourceTypeURL());
		// register endpoint URLs in AbstractResourceEndpoint since they are called with
		// in the API
		this.registerEndpointURLs();
		// register the charon related configurations
		this.registerCharonConfig();
	}
	...
	private void registerCharonConfig() throws CharonException {
		try {
			...
			// config charon
			// this values will be used in /ServiceProviderConfigResource endpoint and some
			...
			log.warn(scimConfigProcessor.getProperties());
			log.warn(String.format("BULK_MAX_OPS: %s",
					scimConfigProcessor.getProperty(SCIMCommonConstants.BULK_MAX_OPERATIONS)));
			charonConfiguration
					.setDocumentationURL(scimConfigProcessor.getProperty(SCIMCommonConstants.DOCUMENTATION_URL));
			...
			charonConfiguration.setChangePasswordSupport(Boolean
					.parseBoolean(scimConfigProcessor.getProperty(SCIMCommonConstants.CHNAGE_PASSWORD_SUPPORTED)));
			...
			for (AuthenticationSchema authenticationSchema : scimConfigProcessor.getAuthenticationSchemas()) {
				Object[] schema = { authenticationSchema.getName(), authenticationSchema.getDescription(),
						authenticationSchema.getSpecUri(), authenticationSchema.getDocumentationUri(),
						authenticationSchema.getType(), authenticationSchema.getPrimary() };
